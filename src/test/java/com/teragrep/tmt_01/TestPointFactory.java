...
public final class TestPointFactory {
    ...
    public TestPointFactory() {
        this(new LazySodiumJava(new SodiumJava()));
    }

    public TestPointFactory(final LazySodiumJava lazySodiumJava) {
        this.lazySodiumJava = lazySodiumJava;
    }

    public RistrettoPoint deterministicPoint(int seed) {
        byte[] hash64 = new byte[Ristretto255.RISTRETTO255_HASH_BYTES];
        ByteBuffer.wrap(hash64).putInt(seed);
        byte[] point = new byte[Ristretto255.RISTRETTO255_BYTES];
        Ristretto255.RistrettoPoint.fromBytes(lazySodiumJava, point);
        lazySodiumJava.cryptoCoreRistretto255FromHash(point, hash64);
